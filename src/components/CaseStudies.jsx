...
const CaseStudyCard = ({ data }) => (
  <div className="bg-white rounded-xl shadow-lg hover:shadow-blue-400/50 transition-shadow duration-300 flex flex-col h-full overflow-visible">
    ...
    <div className="px-6 pb-6 pt-0 md:px-8 md:pb-8 flex flex-col flex-grow">
      <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">{data.title}</h2>
      <p className="text-gray-600 text-base mb-6 flex-grow">{data.description}</p>
  ...
  return (
    <div className="min-h-screen bg-[#DCDEF8] p-4 sm:p-8 font-['Inter'] overflow-x-hidden py-4 px-4">
      ...
      <div className="max-w-6xl mx-auto w-full">
        <div
          ref={carouselRef}
          className="flex overflow-x-scroll snap-x snap-mandatory space-x-6 pb-4 md:pb-6 scroll-smooth no-scrollbar"
        ...
        >
          {caseStudiesData.map((data, index) => (
            <div
              key={index}
              id={`card-${index}`}
              className="flex-shrink-0 snap-start w-[95%] md:w-[calc(50%-12px)]"
            >
      ...
      <div className="flex justify-center mt-8 space-x-2">
        {caseStudiesData.map((_, index) => (
          <div
            ...
            className={`h-3 w-10 rounded-full cursor-pointer transition-all duration-300 ${
